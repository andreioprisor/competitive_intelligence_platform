...
class GoogleAdsScraperPipeline:
    ...
    def __init__(self, serpapi_key: Optional[str] = None, apify_token: Optional[str] = None):
        """
        ...
        """
        self.serpapi_key = serpapi_key or os.getenv("SERPER_API_KEY")
...
if __name__ == "__main__":
    async def main():
        ...
        advertiser_info = await scraper.search_advertiser("adobe.com")
        
        # Scrape ads with custom parameters
        results = await scraper.scrape_ads(
            advertiser_id=advertiser_info["advertiser_id"],
            region=advertiser_info["region"],
            results_limit=20,
            use_ocr=True,
            preset_date="Last+30+days"
        )
